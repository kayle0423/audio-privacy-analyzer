...
流程：
計算每條敏感資訊的出現次數。
遍歷 Whisper 轉錄的每個語音片段 (segment)。
將語音片段中的每個單字進行清理（移除標點符號）。
嘗試在清理後的單字序列中找到完整的敏感資訊短語。
特別處理如 "the", "about" 等可能出現在敏感資訊前的介詞或副詞，進行偏移處理，以確保時間戳記精確地捕捉敏感資訊本身。
如果短語完全匹配，則記錄其類型、內容及在原始音訊中的開始和結束時間。
如果直接短語匹配失敗，會嘗試尋找連續單字連接形成敏感資訊的情況（例如，當 Whisper 將 "ID number" 轉錄為 "ID" 和 "number" 兩個單字時，它仍能識別出它們的組合）。
使用 used_indices 集合來追蹤已匹配的單字索引，防止重複匹配同一段語音。
...
流程：
初始化 sensitive_info 列表（存放結果）和 time_matcher_list（記錄已識別實體的字元範圍，用於重疊檢查）。
首先應用所有預定義的正規表達式 (RE_PATTERNS_phrase) 尋找特定格式的敏感資料。找到的匹配會被添加到 sensitive_info 和 time_matcher_list。
使用客製化訓練的 spaCy 模型 (self.nlp) 對文本進行處理，識別出命名實體。
遍歷客製化模型識別出的每個實體，如果它不與現有匹配重疊，並且不是已被正規表達式處理的類型（如 ZIP、PHONE），則將其添加到 sensitive_info。特別處理了人名和醫生名的判斷（例如，檢查 "Dr. " 前綴）。
使用通用 spaCy 模型 (self.nlp_trf) 對文本進行處理，進一步識別命名實體。
遍歷通用模型識別出的每個實體，如果它不與現有匹配重疊，則將其添加到 sensitive_info。這裡處理了 PERSON、ORG (組織，特別是包含 "department" 或 "hospital" 的組織會被進一步歸類) 和 DATE (特別是星期幾) 等實體類型。
...
流程：
獲取檔案名稱。
使用 self.model.transcribe() 將音訊檔案轉錄成文字，並獲取單字級別的時間戳記。
拼接所有轉錄的文本，並進行時間格式校正。
呼叫 self.extract_sensitive_entities() 從文本中提取敏感資訊。
列印轉錄文本和識別出的敏感資訊。
返回敏感資訊列表、原始轉錄結果和檔案名稱。
...
def process_folder(self, folder_path):
    ...
    for file_path in file_paths:
        ...
        # ... (可選的檔案寫入部分) ...
流程：
讀取資料夾中的所有檔案，並根據檔名（假設為數字）進行排序。
遍歷每個檔案的路徑。
對每個檔案呼叫 self.analyze_audio() 進行分析。
呼叫 self.time_stamp() 獲取每個敏感資訊的精確時間戳記。
將所有時間戳記按其開始時間排序。
列印排序後的時間戳記。
包含被註解掉的程式碼，用於將結果寫入 task2_answer.txt 文件中（可根據需要啟用）。
...
創建 AudioAnalyzer 類別的一個實例。
定義 folder_path 變數，你需要將其設定為你存放音訊檔案的實際路徑。
呼叫 analyzer.process_folder(folder_path) 開始處理指定資料夾中的所有音訊檔案。
使用方式
安裝完成後，你可以輕鬆分析你的音訊檔案。

準備你的音訊檔案： 將所有要分析的音訊檔案 (例如：.wav, .mp3) 放入一個資料夾中。為了方便依序處理，建議將檔案命名為數字 (例如：1.wav, 2.mp3 等)。
